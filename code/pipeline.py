...
from typing import List, Tuple, Optional, Iterable, Union, Callable, Any, Dict, ContextManager, Set, Sized
...
class RunHook(ContextManager):
    ...
    def __init__(self, run_obj: exp.Run, trial_obj: Optional[trial.BaseTrial], seed: Optional[int] = None,
                           n_jobs: int = -1):
        self.run_obj = run_obj
    ...
    def _clear_suffixes(self, to_simplify: Iterable[str]) -> Dict[str, List[Tuple[str, str]]]:
        if not self.remove_suffix:
    ...
    def flush_metrics(self, metrics_to_flush: Optional[Union[str, Iterable[str]]] = None) -> 'RunHook':
        ...
        metrics_to_flush: Dict[str, List[Tuple[str, str]]] = self._clear_suffixes(metrics_to_flush)
