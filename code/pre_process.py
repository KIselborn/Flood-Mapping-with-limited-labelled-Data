...
class HSVDataset(_EOImageExtractor):
    def __init__(self, wrapped: data.Dataset,
                 # WARNING: Normalise is expected to be related to an already offset corrected channel
                 channel_normalisations: Tuple[float, float, float],
                 channel_offsets: Tuple[float, float, float],
                 image_dims: Tuple[int, int, int]):
        super().__init__(wrapped, image_dims)
        self.channel_normalisations = channel_normalisations
...
def default_feature_space_construction(data_folder: str, filters: Optional[List[str]], use_datadings: bool,
    ...
    for filter_method in all_filters:
        train_cons = construct_feature_spaces(data_folder, split=SPLIT_TRAIN, in_memory=True,
                                              filter_method=filter_method, use_datadings=use_datadings,
                                              add_optical=True, as_array=True)
        valid_cons = construct_feature_spaces(data_folder, split=eval_split, in_memory=True,
                                              filter_method=filter_method, use_datadings=use_datadings,
                                              add_optical=True, as_array=True)
