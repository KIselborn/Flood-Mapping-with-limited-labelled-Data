...
@numba.njit(numba.float32[:, ::1](numba.float32[:, :, :, ::1]))
...
@numba.njit(numba.types.UniTuple(numba.float32[::1], 3)(numba.float32[::1], numba.float32[::1], numba.float32[::1]),
            fastmath=False,
            parallel=True)  # )
...
def get_class_constructor(class_name: str) -> Callable:
    ...
    def recursive_constructor_search(cur_module, remaining_split) -> Callable:
        try:
            ...
            return recursive_constructor_search(cur_module, remaining_split)

    return recursive_constructor_search(module, split[1:])


@numba.njit()
def find_first_index(item: int, ar: np.ndarray) -> int:
    ar = ar.flatten()
...
class StoreDictKeyPair(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        res_dict = {(k if not ',' in k else tuple(k.split(','))): (int(v) if v.isdigit() else v) for k, v in map(lambda kv: kv.split('=', maxsplit=1), values)}
