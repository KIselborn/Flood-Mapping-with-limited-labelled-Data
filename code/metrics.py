...
class MetricComputation(JSONSerializable):
    ...
    def forward(self, step: Optional[int] = None, final: bool = True, **kwargs) -> Dict[str, any]:
        ...
        if relevant_vals and any(map(lambda arg: isinstance(arg, torch.Tensor), relevant_vals)):
            kwargs = {k: v.detach().cpu() for k, v in kwargs.items()}
...
class ContingencyMatrixComputation(MetricComputation):
    def __init__(self, name: str, check_predictions: bool = True, check_labels: bool = True) -> None:
        super().__init__(name)
    ...
    def merge_contingency_torch(self, cm: torch.Tensor, ul: torch.Tensor, up: torch.Tensor):
        ...
        if not torch.all(torch.isin(ul, sul)) or not torch.all(torch.isin(up, sup)):
            new_sul = torch.unique(torch.cat((ul, sul)), sorted=True)
            new_sup = torch.unique(torch.cat((ul, sul)), sorted=True)
    ...
    def merge_contingency_numpy(self, cm: np.ndarray, ul: np.ndarray, up: np.ndarray):
        ...
        if not np.all(np.isin(ul, sul)) or not np.all(np.isin(up, sup)):
            ...
            new_sup = np.unique(np.concatenate((ul, sul)))
    ...
    def _compute_torch(self, kwargs: Dict[str, Any], final: bool) -> Dict[str, Any]:
        ...
        if self._use_numpy(predictions.shape, labels.shape):
            ...
            v_tup = self._compute_contingency_torch(predictions=predictions,
                                                    labels=labels)
            self.merge_contingency_torch(*v_tup)
            return self._contingency_to_dict()
...
class ForceNumpy(MetricComputation):
    ...
    def forward(self, step: Optional[int] = None, final: bool = True, **kwargs) -> Dict[str, any]:
        return {k: (v.cpu().numpy() if isinstance(v, torch.Tensor) else v) for k, v in kwargs.items()}
...
_perform_ncpc_computation_numba = numba.njit(parallel=False)(_perform_ncpc_computation)
_perform_ncpc_computation_numba_parallel = numba.njit(parallel=True)(_perform_ncpc_computation)
...
def accumulate_contingency(contingencies: List[Tuple[np.ndarray, np.ndarray, np.ndarray]], exclude_clouds: bool) -> Dict[str, np.ndarray]:
