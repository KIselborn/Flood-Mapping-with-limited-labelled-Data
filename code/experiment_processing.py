...
# file_dir = path.dirname(path.join(path.abspath(os.getcwd()), __file__))
# code_dir = path.dirname(file_dir)
# print(sys.path, 'Appending', code_dir)
# sys.path.append(code_dir)
from utils import get_class_constructor
...
def do_metric_computation(step: int, kwargs: Dict[str, Any], metric_options: MetricOptions) -> Dict[str, Any]:
    ...
    if KEY_CONTINGENCY_MATRIX in kwargs and KEY_CONTINGENCY_UNIQUE_LABELS in kwargs and KEY_CONTINGENCY_UNIQUE_PREDICTIONS in kwargs:
        ...
        contingency_computations = [LabelDistribution('label_distribution')]
...
def accumulate_contingency(contingencies: List[Tuple[np.ndarray, np.ndarray, np.ndarray]],
                           exclude_clouds: bool) -> Dict[str, np.ndarray]:
    def add_mask_safely(cm: np.ndarray, contingency: np.ndarray, labels: np.ndarray, val: int, index: int):
        mask = labels == val
        if np.any(mask):
            cm[index] += contingency[mask].squeeze()

    cm = np.zeros(((2 if exclude_clouds else 3), contingencies[0][2].shape[0]), dtype=np.int64)
    ...
    return {KEY_CONTINGENCY_MATRIX: cm,
            ...
            KEY_CONTINGENCY_UNIQUE_PREDICTIONS: contingencies[0][2]}
...
def process_pipeline(pipe_obj: SerializedObject, pipe_info: BuildingPipeInfo, loaded_files: Dict[str, Any],
    ...
    if issubclass(pipe_obj.actual_type, MultiMetaModule) or issubclass(pipe_obj.actual_type, MetaModule):
        ...
        elif issubclass(pipe_obj.actual_type, StandardizationModule) or \
                issubclass(pipe_obj.actual_type, RangeClippingNormalizationModule) or \
                issubclass(pipe_obj.actual_type, ShapelessInMemoryModule) or \
                issubclass(pipe_obj.actual_type, InMemoryModule) or \
                issubclass(pipe_obj.actual_type, TerminationModule) or \
                issubclass(pipe_obj.actual_type, AssemblerModule) or \
                issubclass(pipe_obj.actual_type, MaskModule) or \
                issubclass(pipe_obj.actual_type, NoCachePipeline) or \
                issubclass(pipe_obj.actual_type, RemoveFromSummaryModule) or \
                issubclass(pipe_obj.actual_type, RetainInSummaryModule):
            pass
        else:
...
def process_entry(tar: tarfile.TarFile, to_process: tarfile.TarInfo, ex_info: ExperimentInfo, skip_folders: List[str],
    ...
    if to_process.isdir():
        ...
        # The trials file is always named "trials.csv"
            ...
            raise RuntimeError(f'Cannot handle file {to_process.name}!')
    else:
...
def parse_tar(tar_file: str, database: PandasDatabase, metric_config: MetricConfiguration) -> ExperimentInfo:
    ...
    with tarfile.open(tar_file, 'r:gz' if tar_file.endswith('gz') else 'r') as tar:
        ...
        skip_files = [prefix + '/trial_copy.json', prefix + '/trial_copy.csv',
                      prefix + '/trial_distributions_copy.json']
        ex_info = ExperimentInfo(pd.DataFrame(), {}, [], [], name, ex_id, seed)
        while next_entry is not None:
            new_entry = process_entry(tar, next_entry, ex_info, skip_folders, skip_files, metric_config)
