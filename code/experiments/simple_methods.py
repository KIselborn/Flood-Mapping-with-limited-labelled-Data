...
def execute_grid_search_experiments(data_folder: str, experiment_base_folder: str,
    ...
    def config(trial: ConditionedTrial):
        ...
        method = trial.suggest_categorical('method', ['GB-RF', 'SGDClassifier'], condition=True, enter=False)#
        feature_space = trial.suggest_categorical('z_feature_space', f_names[filter], condition=True, enter=False)  # )
        trial.enter_condition(['z_feature_space', 'method'])
        num_leaves = None
        if method == 'GB-RF':
