...
def _read_dem_data(as_numpy=False) -> Callable:
    def perform_read(f_name: str) -> Optional[Union[torch.Tensor, numpy.ndarray]]:
        ...
        return dem_data if as_numpy else torch.from_numpy(dem_data)
...
if __name__ == '__main__':
    # BASE_FOLDER = 'data'
    # dataset = read_s2_images(path.join(BASE_FOLDER, 'S2Hand'), label_folder=path.join(BASE_FOLDER, 'label'),
    #                          split_csv_file=path.join(BASE_FOLDER,  'split', 'flood_train_data_s2.csv'),
    #                          filter_bolivia=False)
    # first_data_tensor, first_label_tensor = dataset[1], None
    # if isinstance(first_data_tensor, tuple):
    #     first_data_tensor, first_label_tensor = first_data_tensor
    # print(first_data_tensor.shape, "None" if first_label_tensor is None else first_label_tensor.shape)
    # print(first_data_tensor)
    # print(first_label_tensor)
    # for i in range(first_data_tensor.shape[0]):
    #     plt.imshow(first_data_tensor[i])
    #     # plt.savefig(f"data_channel_{i}.png")
    #     plt.show()
    #     plt.close()
    # if first_label_tensor is not None:
    #     for i in range(first_label_tensor.shape[0]):
    #         plt.imshow(first_label_tensor[i])
    #         plt.show()
    #         # plt.savefig(f"label_channel_{i}.png")
    #         plt.close()
    parser = argparse.ArgumentParser(description='Creates the missing split files for the sentinel-2 hand labeled data')
    parser.add_argument('split_folder', type=str)
    args = parser.parse_args()
    create_s2_split_files(args.split_folder)
