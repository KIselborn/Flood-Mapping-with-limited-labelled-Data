...
def plot_region_comparison(db: PandasDatabase, df: pandas.DataFrame, folder: str,
    ...
    fig, axes, ld = create_figure_with_subplots('Regionwise Metrics', len(metrics), 3)
    ...
    for m_name, flood_label, title, x_positions, y_positions in to_sci_analyze:
        ...
        # TODO save analysis output to file
        analyze(np.log(x_positions), y_positions, xname=flood_label, yname=m_name, title=title)




def query_test(args: argparse.Namespace):
    def filter_fun(d: pd.DataFrame) -> bool:
        return d.loc[d['class_name'] == 1, 'value'].sum() > 0
    db = PandasDatabase(args.database_folder)
    # dist_table_name = 'per_data.label_distribution.no_clouds'
    # metric_ids = find_closest_metric_for_module(db, 'pipeline.UnsupervisedSklearnAdaptorModule',
    #                                             experiment_name='landuyt_reproduce_grid_search',
    #                                             module_param_filter={
    #                                                 'transformer': 'sklearn.cluster.KMeans',
    #                                                 'do_fit': True, 'do_predict': True})
    # merged = merge_module_with_metrics(metric_ids, COLUMN_PIPELINE_SUCCESSOR, dist_table_name)
    # simplified = merged.drop(columns=['module_id', 'metric_id', 'metric', 'predecessor','successor', 'data_id', 'step_id'])
    # print('Distribution', simplified.filter(filter_fun).describe())
    metric_ids = find_closest_metric_for_module(db, 'pipeline.UnsupervisedSklearnAdaptorModule',
                                                experiment_name='landuyt_reproduce_grid_search',
                                                module_param_filter={
                                                    COLUMN_SKLEARN_ADAPTOR_TRANSFORMER_NAME: 'classification.OtsuPerImageThresholdingClassifier',
                                                    'do_fit': False, 'do_predict': True},
                                                merge_module_params=True)
    # clear database and free memory
    db.save()
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.f1_score.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('Otsu-F1', scored_module_ids.groupby('class_name').describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.accuracy.no_clouds', False)
    merged = merge_adaptors_with_transformers(db, scored_module_ids)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('Otsu-Accuracy', scored_module_ids.describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR, 'per_data.iou0.no_clouds',
                                                  False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('Otsu-IOU0', scored_module_ids.groupby('class_name').describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR, 'per_data.iou1.no_clouds',
                                                  False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('Otsu-IOU1', scored_module_ids.groupby('class_name').describe())

    # and now with KI Thresholding
    metric_ids = find_closest_metric_for_module(db, 'pipeline.UnsupervisedSklearnAdaptorModule',
                                                experiment_name='landuyt_reproduce_grid_search',
                                                module_param_filter={
                                                    'transformer': 'classification.KIPerImageThresholdingClassifier',
                                                    'do_fit': False, 'do_predict': True})
    db.save()
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.f1_score.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KI-F1', scored_module_ids.groupby('class_name').describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.accuracy.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KI-Accuracy', scored_module_ids.describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR, 'per_data.iou0.no_clouds',
                                                  False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KI-IOU0', scored_module_ids.groupby('class_name').describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR, 'per_data.iou1.no_clouds',
                                                  False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KI-IOU1', scored_module_ids.groupby('class_name').describe())

    # and for good measure let's check how good it was after KMeans
    metric_ids = find_closest_metric_for_module(db, 'pipeline.UnsupervisedSklearnAdaptorModule',
                                                experiment_name='landuyt_reproduce_grid_search',
                                                module_param_filter={
                                                    'transformer': 'sklearn.cluster.KMeans',
    ...
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.f1_score.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KMeans-MaxAgree-F1', scored_module_ids.groupby('class_name').describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.accuracy.max_agree.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KMeans-MaxAgree-Accuracy', scored_module_ids.describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.iou0.max_agree.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KMeans-MaxAgree-IOU0', scored_module_ids.groupby('class_name').describe())
    scored_module_ids = merge_module_with_metrics(db, metric_ids, COLUMN_PIPELINE_SUCCESSOR,
                                                  'per_data.iou1.max_agree.no_clouds', False)
    scored_module_ids = value_class_seed_only(scored_module_ids)
    print('KMeans-MaxAgree-IOU1', scored_module_ids.groupby('class_name').describe())
...
if __name__ == '__main__':
    ...
    parser.add_argument('-coi','--class_of_interest', dest='class_of_interest', default=None)
    ...
    parser.set_defaults(save_query=True, plot_regions=False)
