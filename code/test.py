...
from clustering import _PerImageLabelNormalizer, _ConnectivityLabelingWrapper, FaissKMeans
from metrics import *
...
def test_label_normalizer():
    ...
    # 1
    # 1]]
    ar = np.array([[[10, 3, 6], [10, 5, 6], [5, 5, 5]]])
    dataset = ArrayInMemoryDataset(ar)
    normalized_datset = _PerImageLabelNormalizer(dataset)
    print(ar)
    print(normalized_datset[0])
...
def assembler_test1():
    features = ArrayInMemoryDataset(np.array([[[0.1, 0.2, 1], [-0.1, 0.3, 1], [-1, 0.1, 1]]]))
    labels = ArrayInMemoryDataset(np.array([[[1, 2, 3], [1, 2, 3], [1, 1, 3]]]))
    labels = ArrayInMemoryDataset(_PerImageLabelNormalizer(labels))
    print('features:', features.data)
    print('labels:', labels.data)
    zonal_stats = _ZonalStatsExtractor(labels, features, ['mean_intensity'])
    zonal_stats = ArrayInMemoryDataset(zonal_stats)
    print('zonal_stats:', zonal_stats.data)
    print('For reference also the labels:', ArrayInMemoryDataset(_ZonalStatsExtractor(labels, None, ['label'])))
    assembled = AssemblerDataset(labels, zonal_stats)
    print('assembled:', ArrayInMemoryDataset(assembled).data)
...
def assembler_test2():
    ...
    print('features:', features.data)
    print('labels:', labels.data)
    stats = _StatsExtractor(labels, features, ['mean_intensity'])
    stats = ArrayInMemoryDataset(stats)
    print('zonal_stats:', stats.data)
    print('For reference also the labels:', ArrayInMemoryDataset(_StatsExtractor(labels, None, ['label'])))
    assembled = AssemblerDataset(labels, stats)
    print('assembled:', ArrayInMemoryDataset(assembled).data)
...
def contingency_matrix_comp_test(test_amount=(1, 512, 512), min_label=-1, max_label=1):
    test_amount = test_amount if isinstance(test_amount, tuple) else tuple(test_amount)
    pred_ints = np.random.randint(0, 2, test_amount)
    label_ints = np.random.randint(min_label, max_label + 1, test_amount)
    pred_ints = ArrayInMemoryDataset(
        _ConnectivityLabelingWrapper(ArrayInMemoryDataset(pred_ints.reshape((1,) + test_amount)))).data[0]
    # n_pred_labels = np.unique(pred_ints).shape[0]
    # print(f'Intial connectivity produced {n_pred_labels} prediction labels. Merging.')
    # pred_ints = merge_neighbouring(n_pred_labels, 3000, pred_ints)
    print(
        f'Starting Numpy computation on {np.unique(pred_ints).shape[0]} prediction labels and {max_label - min_label + 1} actual labels.')
    t = time.time()
    np_contingency, np_unique_labels, np_unique_pred = \
        ContingencyMatrixComputation._compute_contingency_numpy(pred_ints, label_ints, True, True)
    t = time.time() - t
    print(f'Starting Torch computation. Numpy took {t:.3f}s.')
    conv_pred_ints, conv_label_ints = torch.from_numpy(pred_ints), torch.from_numpy(label_ints)
    t1 = time.time()
    torch_contingency, torch_unique_labels, torch_unique_pred = \
        ContingencyMatrixComputation._compute_contingency_torch(conv_pred_ints, conv_label_ints)
    t1 = time.time() - t1
    print(f'Numpy took {t:.3f}s whilst pytorch took {t1:.3f}s')
    assert np.all(np_contingency == torch_contingency.numpy())
    assert np.all(np_unique_pred == torch_unique_pred.numpy())
    assert np.all(np_unique_labels == torch_unique_labels.numpy())
    assert np_contingency.sum() == func.reduce(lambda a, b: a * b, test_amount)
    print('labels     :', label_ints)
    print('predictions:', pred_ints)
    print(np_unique_labels, np_unique_pred)
    print('label X prediction contingency')
    print(np_contingency)
...
def test_faiss_knn():
    ...
    t1 = time.time() - t
    print(f'Not predicting with sklearn.')#print(f'Predicting with sklearn.')
    t = time.time()
...
def test_faiss_kmeans():
    import sklearn.cluster as cluster
    np.random.seed(42)
    index_data = np.random.random((256 * 512*512, 13))
    index_data[0:128*512*512] += 100
    index_data[128*512*512:] -= 100
    query_data = np.random.random((90 * 512*512, 13))
    query_data[0:45*512*512]+=100
    query_data[45*512*512:]-=100
    print('Fitting Faiss')
    t = time.time()
    faiss = FaissKMeans(128, use_gpu=True, verbose=True, random_state=42)
    faiss.fit(index_data)
    t1 = time.time() - t
    print(f'Took {t1:.3f}s. Fitting sklearn')
    t = time.time()
    ref_classifier = cluster.MiniBatchKMeans(128, n_init=3, random_state=42, batch_size=3072, compute_labels=True)
    ref_classifier.fit(index_data)
    t2 = time.time() - t
    print(f'Took {t2:.3f}s for sklearn and gave inertia {ref_classifier.inertia_}.')
    print(f'Predicting with Faiss.')
    t = time.time()
    pred_faiss = faiss.predict(query_data)
    t1 = time.time() - t
    print(f'Took {t1:.3f}s. Predicting with sklearn.')
    t = time.time()
    pred_sklearn = ref_classifier.predict(query_data)
    t2 = time.time() - t
    print(f'Took {t2:.3f}s for sklearn.')
