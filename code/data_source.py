...
VALID_TYPES = {TYPE_LABEL, TYPE_S1, TYPE_S2, TYPE_S1_WEAK_LABEL, TYPE_S2_WEAK_LABEL}
...
class Sen1Floods11DataDingsDatasource(DataSource):
    def __init__(self, base_folder: str, type: str, split: str, use_numpy=True, in_memory: bool = True,
                 buffering: int = 0, chunk_size: int = 256, as_array: bool = False):
        type, split = validate_type_and_split(type, split)
        self.split = split
        self.channel_names = [type]
        if 'label' == type:  # must be one of the hand labeled splits
        ...
        if type.lower() in {'label', 's1', 's2'}:
            ...
            assert type == TYPE_S1_WEAK_LABEL or type == TYPE_S2_WEAK_LABEL
            type = 'label_'+type[:2].lower()
        self.f_name = split + '_' + type
        self.base_folder = base_folder
    ...
    def __call__(self, dataset: Optional[data.Dataset], meta: Optional[pl.Meta]) -> Tuple[data.Dataset, pl.Meta]:
        ...
        if dataset is not None:
            self.print(meta, 'Overwriting already provided dataset!!!', True)
        f_name = path.join(self.base_folder, self.f_name, self.f_name + '.msgpack')
        ...
        if self.in_memory:
            ...
            per_item_info = [pl.PerItemMeta(data['region'], data['key']) for data in dataset]
            del dataset
            dataset = datadings.torch.Dataset(reader, transforms=lambda d: d['image'])
