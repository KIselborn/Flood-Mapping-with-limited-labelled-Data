...
def plot_metric_scatter_plot(ax:plt.Axes, values: pd.DataFrame, metric_names: List[str], clazz: Optional[int],
                             remaining_columns: Union[str, Iterable[str]] = STANDARD_RETAIN_COLUMNS,
                             add_stds: bool = False, plot_regression_line: bool = True):
...
def plot_general_statistics(db: PandasDatabase, folder: str, args: argparse.Namespace, orig_metric_modules: pd.DataFrame):
    ...
    for metric in metrics:
        ...
        for i, clazz in enumerate(unique_classes):
            ...
            plot_metric_histogram(axes[0, i], modules_to_use, metric, clazz)
        if scatter_plot:
            plot_class_scatter_plot(axes[0, -1], config_metric_modules, metric, unique_classes,
                                    add_stds=args.std_errors)
...
if __name__ == '__main__':
    ...
    parser.set_defaults(show=True, save=True, std_errors=False, plot_general=True, plot_combinations=True, plot_params=True)
    parser.set_defaults()
    ...
    if args.plot_general:
        plot_general_statistics(db, folder, args, orig_metric_modules)
